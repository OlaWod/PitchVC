...
const static float MAX_WAV_VALUE = 32768.0;
...
class WavReader {
 public:
  ...
  bool Open(const std::string& filename) {
    ...
    if (NULL == fp) {
      fprintf(stderr, "Error in read %s", filename.c_str());
      return false;
    }
    ...
    if (header.fmt_size < 16) {
      ...
      if(fread(header.data, 8, sizeof(char), fp)){};
    }
    ...
    for (int i = 0; i < num_data; ++i) {
      switch (bits_per_sample_) {
        case 8: {
          char sample;
          if(fread(&sample, 1, sizeof(char), fp)){};
          data_[i] = static_cast<float>(sample);
          break;
        }
        case 16: {
          int16_t sample;
          if(fread(&sample, 1, sizeof(int16_t), fp)){};
          data_[i] = static_cast<float>(sample);
          break;
        }
        case 32: {
          int sample;
          if(fread(&sample, 1, sizeof(int), fp)){};
          data_[i] = static_cast<float>(sample);
          break;
        }
        default:
