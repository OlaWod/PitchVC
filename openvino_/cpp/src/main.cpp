...
int str2int(const std::string& inp) {
    int out = static_cast<int>(std::stoull(inp.substr(0, inp.size())));
    return out;
}
...
void infer(const std::string& title, const std::string& wav_path, const std::string& spk, int f0_shift,
    ...
    tfft::TorchSTFT torchstft(16, 4, 16);
    tfft::TorchMel torchmel(assets_dir);
    ...
    FormatReader::ReaderPtr spk_emb_reader(spk_emb_path.c_str());
    if (spk_emb_reader.get() == nullptr) {
        std::stringstream ss;
        ss << spk_emb_path.string() + " cannot be read!";
        throw std::logic_error(ss.str());
    }
    std::shared_ptr<unsigned char> spk_emb_data = spk_emb_reader->getData();
    ...
    // ov tensors
    ov::Tensor wav_tensor_ov = wrap_torch_tensor_as_ov(wav_tensor_torch);
    ov::Tensor mel_tensor_ov = wrap_torch_tensor_as_ov(mel_tensor_torch);
    ...
    // -------- Step 3. stft --------
    torch::Tensor har_source_tensor_torch = wrap_ov_tensor_as_torch(har_source_tensor_ov);
    ...
    ov::Tensor har_spec_tensor_ov = wrap_torch_tensor_as_ov(har_spec_tensor_torch);
    ov::Tensor har_phase_tensor_ov = wrap_torch_tensor_as_ov(har_phase_tensor_torch);
    ...
    // -------- Step 5. istft --------
    torch::Tensor spec_tensor_torch = wrap_ov_tensor_as_torch(spec_tensor_ov);
    torch::Tensor phase_tensor_torch = wrap_ov_tensor_as_torch(phase_tensor_ov);
    ...
    try {
        ...
        for (const auto& line : lines) {
            ...
            if (std::getline(iss, title, '|') &&
                ...
                candidates.push_back(std::make_tuple(title, src_wav, tgt_spk, str2int(f0_shift)));
            } else {
