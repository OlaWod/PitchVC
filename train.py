...
def train(rank, a, h):
    ...
    trainset = MelDataset(training_filelist, h.segment_size, h.n_fft, h.num_mels,
                          ...
                          shuffle=False if h.num_gpus > 1 else True, fmax_loss=h.fmax_for_loss, device=device, use_aug=h.use_aug)
    ...
    if rank == 0:
        validset = MelDataset(validation_filelist, h.segment_size, h.n_fft, h.num_mels,
                              h.hop_size, h.win_size, h.sampling_rate, h.fmin, h.fmax, n_cache_reuse=0,
                              fmax_loss=h.fmax_for_loss, device=device, use_aug=h.use_aug)
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            if rank == 0:
                start_b = time.time()
            if h.use_aug:
                mel_aug, mel, y, _, y_mel, spk_emb, spk_id = batch
                y = torch.autograd.Variable(y.to(device, non_blocking=True))
                mel_aug = mel_aug.to(device, non_blocking=True)
                y4x = vocoder.decode_batch(mel_aug)
                max_v = y4x.abs().max(-1).values.unsqueeze(-1)
                max_v_new = 0.65 + 0.33 * torch.rand_like(max_v) # 0.65 ~ 0.98
                y4x = y4x * max_v_new / max_v
                y4x = y4x.squeeze(1)
            else:
                mel, y, _, y_mel, spk_emb, spk_id = batch
                y = torch.autograd.Variable(y.to(device, non_blocking=True))
                y4x = y
            mel = torch.autograd.Variable(mel.to(device, non_blocking=True))
            y_mel = torch.autograd.Variable(y_mel.to(device, non_blocking=True))
            x = wavlm(y4x).last_hidden_state # (B, T, C=768)
            x = x.transpose(1, 2) # (B, C, T)
            x = F.pad(x, (0, mel.size(2) - x.size(2)), 'constant')
            y = y.unsqueeze(1)
            spk_emb = torch.autograd.Variable(spk_emb.to(device, non_blocking=True))
            spk_id = torch.autograd.Variable(spk_id.to(device, non_blocking=True))
                        ...
                        for j, batch in enumerate(validation_loader):
                            ...
                            if j <= 4:
                                if steps == 0:
                                    ...
                                    sw.add_figure('gt/y_spec_{}'.format(j), plot_spectrogram(mel[0].cpu()), steps)
